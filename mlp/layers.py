...
class LeakyReluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        alpha=0.01
        return np.where(inputs<0,alpha*inputs,inputs)        
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        alpha=0.01
        return np.where(outputs<0, alpha,1)*grads_wrt_outputs
...
class RandomReluLayer(Layer):
    ...
    def __init__(self, lower=0.125, upper=0.3333333333333333, rng=None):
        ...
        #self.alpha=self.rng.uniform(low=self.lower, high=self.upper, size=(50,3,64,64))
    ...
    def fprop(self, inputs, leakiness=None):
        ...
        outputs=np.where(inputs<0,np.multiply(self.alpha,inputs),inputs)
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        grad=np.where(outputs<0,np.multiply(self.alpha,grads_wrt_outputs),grads_wrt_outputs)
        return grad
...
class ParametricReluLayer(LayerWithParameters):
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        inputs=np.minimum(inputs,0)
        sumarr=np.multiply(inputs,grads_wrt_outputs)
        sum=0
       # for i in range(50):
        #    for j in range(3):
         #       for k in range(64):
...
class ExponentialLinearUnitLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        alpha=0.01
        return np.where(inputs<0,alpha*np.exp(inputs)-alpha,inputs)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        alpha=0.01
        return np.where(outputs<0,alpha*np.exp(inputs)*grads_wrt_outputs,grads_wrt_outputs)
